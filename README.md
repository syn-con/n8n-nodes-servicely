...
- **Options → Execution Script** — a script the service desk runs for this tool, exported with it. Every `@@URL@@` in it is replaced with this tool's webhook URL in single quotes when the workflow is activated, so a script can name its own endpoint without being edited per instance — a test listen writes the test URL, since that is the one answering while it runs. A script that uses `@@URL@@` when n8n cannot resolve the URL fails the activation rather than registering a broken script.
