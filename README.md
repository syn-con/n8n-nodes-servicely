...
- **`IsProduction`** — a boolean parameter every tool carries on top of the declared ones, exported last so it never reorders them. Its description tells the agent to send `true` unless the user explicitly asked for a test run, so a workflow can tell a real call from a rehearsal without each tool defining its own flag. It is required like any other parameter, and declaring a row named `IsProduction` replaces it — type, description and position then come from that row.
...
- **Options → Execution Script** — a script the service desk runs for this tool, exported with it. Every `@@URL@@` in it is replaced with this tool's webhook URL when the workflow is activated, so a script can name its own endpoint without being edited per instance. A bare `@@URL@@` comes out in single quotes; one the script already wrapped in `'`, `"` or `` ` `` keeps the quotes it was written with, so `post('@@URL@@')` gives `post('https://…')` rather than a doubled pair — a test listen writes the test URL, since that is the one answering while it runs. A script that uses `@@URL@@` when n8n cannot resolve the URL fails the activation rather than registering a broken script.
