...
/**
 * Answers the agent that called a Servicely AI Tool. Pairs with the
 * {@link import('./ServicelyAITool.node').ServicelyAITool} trigger set to
 * "Using Servicely AI Tool Response Node".
 */
export class ServicelyAIToolResponse implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Servicely AI Tool Response',
		...
		description: 'Respond to the agent that called the Servicely AI Tool',
		documentationUrl: 'https://docs-servicely.atlassian.net/wiki/spaces/SD/pages/2077523978',
		defaults: {
			name: 'Servicely AI Tool Response',
		},
		...
		properties: [
			{
				displayName:
					'The Servicely AI Tool that starts this workflow must have "Respond" set to "Using Servicely AI Tool Response Node". Answer before its Timeout elapses, otherwise the caller already got a 504.',
			...
			},
			{
				...
				description:
					'Optional machine readable details added to the error, e.g. the validation errors of the Servicely AI Tool',
				displayOptions: { show: { respondWith: ['error'] } },
			},
			{
				displayName: 'Options',
