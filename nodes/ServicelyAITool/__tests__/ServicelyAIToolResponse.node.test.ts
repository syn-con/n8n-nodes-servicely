...
import { ServicelyAIToolResponse } from '../ServicelyAIToolResponse.node';

const node = new ServicelyAIToolResponse();
...
async function execute(params: IDataObject = {}, items: IDataObject[] = [{ id: 1 }]) {
	...
	const ctx = {
		...
		getNode: () => ({ name: 'Servicely AI Tool Response' }),
...
describe('node description', () => {
	it('is registered under the Servicely AI Tool name', () => {
		expect(node.description.name).toBe('servicelyAiToolResponse');
		expect(node.description.displayName).toBe('Servicely AI Tool Response');
	});
});
